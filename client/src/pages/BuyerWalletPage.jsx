...
const BuyerWalletPage = () => {
  ...
  const handleSetupPayoutAccount = async (event) => {
    ...
    try {
      ...
      setActionMessage("Payout account setup complete. You can withdraw your available balance now.");
  ...
  const handleWithdrawFinalizedBalance = async () => {
    ...
    try {
      const response = await fetch(`${backendURL}/user/withdraw-finalized-balance`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      setActionMessage("Withdrawal processed successfully.");
  ...
  const filteredTransactions = transactions.filter((txn) => filter === "all" || txn.type === filter);
  ...
  const renderMetaText = (txn) => {
    ...
    return `${txn.counterparty ? `From: ${txn.counterparty}` : "Payment Event"}${txn.orderDeliveryStatus ? ` • Order: ${txn.orderDeliveryStatus}` : ""}`;
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <p>Connect a bank account or UPI ID, then withdraw any available credits from your wallet.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", marginBottom: "20px" }}>
        <div className={styles.transactionCard} style={{ padding: "14px" }}>
          <div>
            <div style={{ fontSize: "0.8rem", color: "#64748b" }}>Available Balance</div>
        ...
        <div className={styles.transactionCard} style={{ padding: "14px" }}>
          <div>
            <div style={{ fontSize: "0.8rem", color: "#64748b" }}>Withdrawal In Progress</div>
            <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "#d97706" }}>Rs. {Number(summary.inProgressWithdrawals || 0).toFixed(2)}</div>
          </div>
        </div>
        <div className={styles.transactionCard} style={{ padding: "14px" }}>
          <div>
            <div style={{ fontSize: "0.8rem", color: "#64748b" }}>Failed Withdrawals</div>
            <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "#dc2626" }}>Rs. {Number(summary.failedWithdrawals || 0).toFixed(2)}</div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.transactionList}>
        {filteredTransactions.length > 0 ? (
          ...
          <div className={styles.emptyState}>
            ...
            <button type="button" className={styles.primaryActionBtn} onClick={() => navigate("/profile")}>
              Back to Profile
            </button>
          </div>
        )}
